...
class CGLDataset(Dataset):
    """CGL dataset with layouts as targets."""
    ...
    def _per_patch_tokenize(
        ...
        is_text = self._cat_id2name(cat_tok.item()) in ["文字", "强调突出子部分文字"]
        if is_text:
    ...
    def __get_order(self, example: Any) -> Dict[str, np.ndarray]:
        ...
        order_dic = {
            ...
            "layer": self._get_layer_order(example, is_ingroup_random=True),
            "layer_raster": self._get_layer_order(example, is_ingroup_random=False),
        }
    ...
    def _get_layer_order(
        self, example: Any, is_ingroup_random: bool = True
    ) -> np.ndarray:
        """Get layer-based order, with configurable intra-layer ordering.

        Layer priority from top to bottom:
        Logo > Text > Embellishment > Underlay > Background

        Args:
            example: Dataset example containing element information
            is_ingroup_random: If True, elements within same layer are randomly ordered.
                            If False, applies raster ordering within each layer.
        ...
        # Define category priority (higher value = higher layer)
        category_priority = {
            "Logo": 5,
            "文字": 4,
            "强调突出子部分文字": 4,
        ...
        # Convert category IDs to priority values
        priorities = np.zeros(N, dtype=np.int32)
        for i in range(N):
            cat_name = self._dataset.features["type"].feature.int2str(
                int(categories[i])
            )
            priorities[i] = category_priority.get(cat_name, 0)

        # Group elements by priority level
        unique_priorities = np.unique(priorities)
        layer_order = np.array([], dtype=np.int32)

        # Process each priority level (from lowest to highest)
        for priority in unique_priorities:
            # Get indices of elements with current priority
            current_layer_indices = np.where(priorities == priority)[0]

            if is_ingroup_random:
                # Randomize ordering within the same layer
                np.random.shuffle(current_layer_indices)
            else:
                # Apply raster ordering within the layer
                if len(current_layer_indices) > 1:
                    # Extract positions for elements in this layer
                    layer_left = np.array(
                        [example["left"][idx] for idx in current_layer_indices]
                    )
                    layer_top = np.array(
                        [example["top"][idx] for idx in current_layer_indices]
                    )

                    # Apply raster ordering (top to bottom, left to right)
                    raster_indices = np.lexsort((layer_left, layer_top))
                    current_layer_indices = current_layer_indices[raster_indices]

            # Append the ordered indices for this layer
            layer_order = np.append(layer_order, current_layer_indices)
